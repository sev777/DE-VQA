...
def compute_hice_multimodal_edit_quality(
    ...
    i1=image
    i2=record['sim_edit_image'].to(hparams.device)
    i3=m_loc_image
    i4=None

    for (tex,img),tag,ans in zip(
                             [[t1,i4],[t2,i4],[t1,i2],[t1,i3],[t2,i1],[t2,i2],[t3,i1],],
                             ['t1i4','t2i4','t1i2','t1i3','t2i1','t2i2','t3i1' ],
                                [target,target,target,target,target,target,m_loc_a]
                             ):
        # print(tex)
        if pre_edit:
            _, ml_preds, locality_image_output = icl_multimodal_lm_eval(model, model_name, hparams, tok, icl_examples,                                                                        ans, tex, img, is_loc=True)
        else:
            _, multimodal_locality_preds = our_predict_domain(m_loc_q, m_loc_a, sentence_model1, Wo, W_rand, device, mloc_memory, threshold)
            if multimodal_locality_preds == 0:
                try:
                    _, _ml_preds, locality_image_output = icl_multimodal_lm_eval(model, model_name, hparams, tok, [''],
                                                                                 ans, tex, img, is_loc=True)
                except:
                    print('error')
            else:
                icl_examples = select_icl_samples(tex, ans, stored_data, sentence_model, device, hparams)
                _, _ml_preds, locality_image_output = icl_multimodal_lm_eval(model, model_name, hparams, tok, icl_examples,
                                                                             ans, f'New Fact: {prompt} {target}\nPrompt: {tex}', img, is_loc=True)
            ret[f'{tag}_acc'] = 1 if multimodal_locality_preds == 0 else 0
            # ret[f'{tag}_output'] = locality_image_output
