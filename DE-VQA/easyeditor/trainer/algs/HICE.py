...
class HICE(EditableModel):
    ...
    def __init__(self, model, config, model_constructor, sentence_model=None, classifier=None,
        ...
        self.sentence_model = sentence_model if sentence_model is not None else SentenceTransformer(
            config.sentence_model_name).to(f'cuda:{config.device}')
        self.classifier = classifier if classifier is not None else nn.Linear(self.embed_dim,
                                                                              self.class_num)  # clip 32:512
    ...
    def edit(self, batch, condition=None, detach_history=False):
        ...
        new_model = HICE(self.model, self.config, self.model_constructor, self.sentence_model, self.classifier,
                         self.sentences, self.sentences_embed, self.icl_examples, train_data, self.Wo, self.W_rand,
                         self.count)
    ...
    def find_icl_examples(self, request):
        ...
        hits = util.semantic_search(query_embedding, stored_embeddings, score_function=util.dot_score,
                                    top_k=self.config.k)
        assert len(hits) == 1
        hit = hits[0]
    ...
    def get_Wo(self):
        ...
        self.Wo = torch.linalg.solve(self.G + ridge * torch.eye(self.G.size(dim=0)),
    ...
    def exact_train_features(self, loader):
        ...
        for i, batch in enumerate(loader):
            ...
            img = batch['edit_inner']['image'][0]
            image = preprocess(Image.open(img)).unsqueeze(0).to(device)
            # image=batch['edit_inner']['image'].unsqueeze(0).to(device)
            batch['edit_inner']['ori_qs'] = [batch['edit_inner']['text_input'][0][:70]]
            text = clip.tokenize(batch['edit_inner']['ori_qs']).to(device)
#./data/val2014/COCO_val2014_000000581929.jpg
            with torch.no_grad():
            ...
            pid.append(i)#batch['edit_inner']['pid'][0])
    ...
    def train_classifier(self, train_loader, test_loader):
        import logging, os
        weight_file = 'ranpac_{}_{}.pth'.format(self.config.task.lower(),
                                                self.config.sentence_model_name.split('/')[-1])

        if os.path.exists(weight_file):
            params = torch.load(weight_file)
            params['Wo'], params['W_rand'] = params['Wo'].to(self.config.device), params['W_rand'].to(
                self.config.device)
        ...
        memory_file = 'memory_{}_{}.pth'.format(self.config.task.lower(),
                                                self.config.sentence_model_name.split('/')[-1])
    ...
    def test_classifier_acc(self, params, test_loader, mloc_memory=None):
        self.fea, self.labels, self.Wo, self.W_rand = params['fea'], params['labels'], params['Wo'].to(
            self.config.device), params['W_rand'].to(self.config.device)
        keys = ["edit_inner", "edit_outer", "loc", "loc_image"]
        accs = []
        for key in keys:
            accs.append([])
