...
class BaseTrainer:
    def __init__(self, config, train_set: Dataset, val_set: Dataset):
        ...
        self.save_path = f"{model_dir}/{self.config.setting}/{safe_model_name}"
    ...
    def save_state(self, stats):
        ...
        obj = {
            ...
            "params":self.config.inner_params,
    ...
    def run(self):
        ...
        for epoch in range(self.epoches):
            for batch in tqdm(self.train_loader, ncols=120, desc=f'Epoch {epoch+1}/{self.epoches}'):
                ...
                if self.global_iter % self.config.val_interval == 0:
                    ...
                    if stopper.update(self.global_iter, val_info) or (best_step ==0 and self.global_iter in [10000,15000,20000,25000]):
        ...
        results_path = f"{self.save_path}/Feval_{self.config.model_name}_results.json"
        # else:
        #     results_path = f"{os.getcwd()}/{cur_time}_{self.config.model_name}_{ self.config.setting}_results.json"
        ...
        with open(results_path, "w") as f:
            ...
            json.dump(
                {"inner_params": self.config.inner_params}, f
            )
