...
class OPTDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if 'chooses' in kwargs:
            if idx in kwargs['chooses']:
                tokens=kwargs['chooses'][idx]
                # 创建一个和 hidden_states 的第二维度一样长的 mask
                mask = torch.zeros(hidden_states.shape[1], dtype=torch.bool).cuda()
                # 将要保留的索引设置为 True
                mask[tokens] = True
                # 利用 broadcasting 将不在列表中的 token 的值改为 0
                hidden_states = hidden_states * mask.unsqueeze(0).unsqueeze(-1)
...
class OPTDecoder(OPTPreTrainedModel):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            all_layer_outputs += (layer_outputs[3],)
        all_hidden_states += (hidden_states,)#add external
