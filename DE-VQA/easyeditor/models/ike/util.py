...
def encode_ike_facts_multimodal(sentence_model: SentenceTransformer, ds: Dataset, hparams: IKEMultimodalHyperParams):
    ...
    for i, train_data in tqdm(enumerate(ds), total=len(ds), ncols=120):
        ...
        neighbors = train_data['locality_prompt']
        neighbors_ans = train_data['locality_ground_truth']
        images.append(train_data['image_path'])
        images.append(train_data['rephrase_image_path'])
        images.append(train_data['locality_image_path'])
        prompts.append([train_data['prompt'] , train_data['target']])
        prompts.append([paraphrases ,target_new])
        prompts.append([neighbors,neighbors_ans])
        sentences.append(f"New Fact: {new_fact}\nPrompt: {new_fact}\n\n")
        sentences.append(f"New Fact: {new_fact}\nPrompt: {paraphrases} {target_new}\n\n")
        sentences.append(f"New Fact: {new_fact}\nPrompt: {neighbors} {neighbors_ans}\n\n")

        # sentences.append(f"{new_fact}")
        # sentences.append(f"{paraphrases} {target_new}")
    ...
    os.makedirs(base_path, exist_ok=True)
    safe_model_name = hparams.sentence_model_name.rsplit('/', 1)[-1]
    print(f'Save to : {base_path}/{hparams.task_name}_embeddings_llava.pkl')
    with open(f'{base_path}/{hparams.task_name}_embeddings_llava.pkl', "wb") as fOut:
        pickle.dump({'sentences': sentences, 'embeddings': embeddings,'images':images,'prompts':prompts}, fOut,
                    protocol=pickle.HIGHEST_PROTOCOL)
